	...
	public void adjustToPresent(TemporalOperator TOp, Time time){
		...
		if(!simulatedTime){
			...
			if(realTimeComp >= 0 )  {	
				...
				since = (until - TOp.getSinceValue().getSimulation_value()/time.getTIME_UNIT_MILLIS()); //New added Gines
			}else{
				since = -1;
			}
...
//			System.out.println("DBG - UNTIL: "+until);
	...
	public void adjustToAbsolute(TemporalOperator TOp, Time time){
		...
		if(simulatedTime){
			...
			long realTime = 0;
			//System.out.println("TOC.java - adjustToAbsolute " + TOp.getUntilValue().isNotFuture(time) );
			
			// Negado por Gines
			if(TOp.getUntilValue().isNotFuture(time)){
				//long now = time.getBaseTimeMillis();
				//realTime has the value of TOP until and since. -1 if is in the future.
				...
				if (until < 0){
					until =  (TOp.getUntilValue().getTimeMillis(time) - time.getBaseTimeMillis())/1000; 
				}
				
				
			//	System.out.println("TOC.java - adjustToAbsolute() set since " + since + " until " + until + " ITERATION " + time.getIteration());
			}else{
				until = -1;
			}
		}
	...
	boolean checkStrongRelative(TemporalOperator TOp, Time time) throws SQLException{
		...
		//System.out.println("Checking strong relative");
	...
	boolean checkStrongAbsolute(TemporalOperator TOp, Time time) throws SQLException{
		...
		//System.out.println("Checkin Strong Absolute  " + (TOp.getUntilValue().getTimeMillis(time) - time.getBaseTimeMillis())/1000);
	...
	boolean checkWeakAbsolute(TemporalOperator TOp, Time time) throws SQLException{
		boolean  isTrueInBetween = false; 
		adjustToAbsolute(TOp,time);
	//	System.out.println("Checkin Weak Absolute " + TOp.getName() + " " + TOp.getSinceValue() + " " + time.getBaseTimeMillis());
	...
	private boolean isAtLeastOneValueDuringInterval(TemporalOperator TOp){
		...
		boolean result;
			...
			//	System.out.println(" 1 SINCE"+since+" UNTIL: "+until);
	...
	private boolean isSameValueDuringWholeInterval(TemporalOperator TOp){
		...
		if(TOp.getType() == TOP_TYPE.STRONG_RELATIVE || TOp.getType() == TOP_TYPE.WEAK_RELATIVE){
			//System.out.println("SAME VALUE DURING WHOLE INTERVAL  since " + since + " until " + until);
			//Gines 
