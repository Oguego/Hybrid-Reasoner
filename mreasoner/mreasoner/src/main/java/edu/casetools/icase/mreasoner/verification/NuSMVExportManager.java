package edu.casetools.icase.mreasoner.verification;
...
public class NuSMVExportManager {
	...
	public void export(String filename, MConfigs configs){
			this.configs = configs;
			try {
				if(configs.getTimeConfigs().getExecutionMode() == EXECUTION_MODE.SIMULATION_ITERATION){
					systemData = loader.readSystemSpecifications_Iteration(configs.getFilesConfigs().getSystemSpecificationFilePath());
	...
	private void exportData(String filename) {
		...
		verificationData.setMaxIteration((int) configs.getTimeConfigs().getExecutionTime());
	    generateStates();
	    generateRules();
