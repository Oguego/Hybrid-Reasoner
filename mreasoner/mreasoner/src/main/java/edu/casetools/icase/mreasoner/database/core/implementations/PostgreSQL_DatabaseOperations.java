...
public class PostgreSQL_DatabaseOperations extends DatabaseOperations{
	...
	public void insertEvent(String stateName, String status, String iteration, String date, String time) {
	    ...
	    try {
			...
			// TODO Auto-generated catch block
			System.out.println("YYYEEEEJAAAA " + query);
			e1.printStackTrace();
		}
...
//		}
	...
	@Override
	public boolean getAtLeastOneValueQueryAbsolute(String since, String until,	String state, String status) {
		
		String query = "SELECT \""+state+"\" FROM \"results\" WHERE iteration >= '"+since+ "' and iteration <= '"+until+"' and \""+ state+"\" = '"+status+"'";
		
		//System.out.println("SQLOPERATIONS - getAtLeastOneValueQueryAbsolute " + query);
	...
	//	System.out.println("getSameValueQueryPresent >> " + query);  
		...
		//System.out.println(query);
			try {
				result = dbConnection.executeQueryOpenStatement(query);
				if(result.next()){
					//System.out.println("RESULTADO -----> " + result.getInt("iterations") + " == " + (Integer.parseInt(until) -  Integer.parseInt(since)));
...
//			}
	...
	@Override
	/**
	 * Return -1 if the iteration is in the future, e.i. the result of query is null 
	 */
	public long getIterationFromRealTime(String realTime) {
		String query;
		ResultSet resultSet;
		long result = -1; //Because 0 is the default value if retunr null. Null means that the iteration its no happend yet but 0 indicate thas happens in 0 iteration.
		String s="";
		query = "SELECT MIN(\"iteration\") FROM results WHERE system_time_millis >= "+realTime;
		//System.out.println("SQL OPERATIONS - getIterationFromRealTime: " + query);
		try {
			resultSet = dbConnection.executeQueryOpenStatement(query);
			if(resultSet.next()){
				s = resultSet.getString(1); //PROBLEM NO DETECT DE UNTIL because return null but getLong set to 0.
				if (!resultSet.wasNull()){
					result = Long.parseLong(s);
				}
			//	System.out.println("SQL OPERATIONS - getIterationFromRealTime: ITERATION DETECTED: "+result );
			}
			resultSet.close();
			dbConnection.closeStatement();
		} catch (SQLException e) {
	...
	public boolean setOutsideActuatorResult(String state, String datetime, String value) {
		...
		query = "INSERT INTO outcomes (state, datetime, value) VALUES ('" + state + "', '" + format.format(timestamp) +"', '" + value + "')";
	//	System.out.println(query);
		try {
			dbConnection.executeUpdate(query);
		} catch (SQLException e) {
	...
	public ResultSet getSensors(){
		...
		ResultSet resultSet=null;
	//	String query = "Select device, state, type, variable  from sensors where type is not null and variable is not null";
		String query =  "Select s.device, s.state, i.type, i.variable, i.class  from sensors s join sensor_implementations i on s.implementation =i.name   where i.type is not null and i.variable is not null";
		try {
			resultSet = dbConnection.executeQueryOpenStatement(query);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
	...
	public boolean hasColumn(String table, String column){
		...
		try {
			...
			while (resultset.next()) {
				//	System.out.println("DB checiking column name in result: %" + resultset.getString(1) + "%");
	...
	@Override
	public ResultSet getSensorActuators() { //GINES
		// TODO Auto-generated method stub
		ResultSet resultSet=null;
		String query = "Select state, class from sensors s join sensor_implementations i on s.implementation=i.name where i.actuator=true";
		try {
			resultSet = dbConnection.executeQueryOpenStatement(query);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
	...
	@Override
	public ResultSet getSensorsName() {
