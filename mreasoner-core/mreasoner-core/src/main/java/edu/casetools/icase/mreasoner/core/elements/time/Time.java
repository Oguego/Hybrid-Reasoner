package edu.casetools.icase.mreasoner.core.elements.time;
...
public class Time {
	...
	public void start(){
		...
		if(!timeIsGivenInIterations) 
			ITERATION_MILLIS = BASE_TIME_MILLIS + (ITERATION * TIME_UNIT_MILLIS); // New Addition
	...
	public boolean endOfTimeUnit() {
		...
		// Gines
		// Return false if it already is in iteration
	//	System.out.print("endtimeunit " + ITERATION_MILLIS + " = " + BASE_TIME_MILLIS + " + " +  ITERATION + " * " +  TIME_UNIT_MILLIS);
		//if(ITERATION_MILLIS >=  BASE_TIME_MILLIS + (ITERATION * TIME_UNIT_MILLIS)) return false;
		else return true;
	}
